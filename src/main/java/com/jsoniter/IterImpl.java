...
class IterImpl {
    ...
    static final int readPositiveInt(final JsonIterator iter, byte c) throws IOException {
        ...
        while(iter.head != iter.tail){
            int d = IterImplNumber.intDigits[IterImpl.nextToken(iter)];
            if(d == IterImplNumber.INVALID_CHAR_FOR_NUMBER){
        	return ind;
            }
            ind *= 10;
            ind += d;
	...
	long ind = IterImplNumber.intDigits[c];
       ...
       while(iter.head != iter.tail){
	   long d = IterImplNumber.intDigits[IterImpl.nextToken(iter)];
	   if(d == IterImplNumber.INVALID_CHAR_FOR_NUMBER){
	       return ind;
	   }
	   ind *= 10L;
